...
export default function AnalysisCreate() {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      showToast(getPostPublishError(err, "Failed to publish analysis"), "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveDraft = () => {
    localStorage.setItem("analysis_draft", JSON.stringify({ text, keywords, sources, audience, authorId: user?.id }));
    ...
    <div className="force-light-page" style={{ background: BG, minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <div className="header-container" style={{ display: "flex", alignItems: "center", padding: "24px 40px 40px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 20, width: "100%", flexWrap: "wrap", justifyContent: "inherit" }} className="header-inner">
          ...
          <h1 className="header-title" style={{ fontSize: 38, fontWeight: 700, color: PRIMARY, margin: 0 }}>Post - Analysis</h1>
        </div>
      </div>
