...
export default function AnalysisCreate() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("analysis_draft");
    if (saved) {
      try {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        showToast("Analysis published", "success");
        localStorage.removeItem("analysis_draft");
        setTimeout(() => navigate("/forum"), 1500);
      }
    } catch (err) {
      showToast("Failed to publish analysis", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveDraft = () => {
    localStorage.setItem(
      "analysis_draft",
      JSON.stringify({ text, keywords, sources, audience, authorId: user?.id })
    );
  ...
  return (
    <div style={{ background: BG, minHeight: "100vh", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex" }}>
        ...
        >
          <textarea
            placeholder="Type analysis here..."
            value={text}
          ...
          <div style={{ marginTop: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <span onClick={addSource} style={{ color: "#4A90E2", cursor: "pointer", fontSize: 14, fontWeight: 600 }}>
                + Add source
              </span>
            </div>
            {sources.map((s, i) => (
              <div key={i} style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  ...
                  {sources.length > 1 && (
                    <span
                      ...
                      style={{ fontSize: 12, color: "#FF4C4C", cursor: "pointer" }}
                    >
                      Remove
                    </span>
                  )}
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  <input
                    ...
                    style={{ flex: 1, height: 44, padding: "0 12px", borderRadius: 10, border: `1px solid ${BORDER}` }}
                  />
                  <input
                    ...
                    style={{
                      ...
                      padding: "0 12px",
                      borderRadius: 10,
                      border: s.error ? "1px solid red" : `1px solid ${BORDER}`,
                    }}
                  />
                </div>
