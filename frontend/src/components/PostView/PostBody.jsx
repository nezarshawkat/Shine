...
function CommentItem({ comment, user, onLike, onDelete, onEdit, onReport }) {
  ...
  return (
    <div
      style={{ display: "flex", gap: 12, marginTop: 15, position: "relative" }}
    >
...
export default function PostBody() {
  ...
  const handlePollVote = async (optionId) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const updated = await fetch(`${BASE_URL}/api/posts/${postId}`, {
    ...
    try {
      if (editingComment) {
        const res = await fetch(
          `${BASE_URL}/api/comments/${editingComment.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      <ImageMaximizer
        ...
        onNext={() =>
          setMaximizedIndex((maximizedIndex + 1) % mediaUrls.length)
        }
        onPrev={() =>
          setMaximizedIndex(
            (maximizedIndex - 1 + mediaUrls.length) % mediaUrls.length,
          )
        }
      />
      ...
      >
        ...
        >
          ...
          >
            <div style={{ fontSize: isMobile ? 13 : 16, color: "#1C274C" }}>
        ...
        >
              ...
              >
                {post.pollOptions.map((opt) => {
                  const totalVotes = post.pollOptions.reduce(
                    (acc, curr) => acc + (curr._count?.votedUsers || 0),
                    0,
                  );
                  const percentage =
                    totalVotes > 0
                      ? Math.round(
                          ((opt._count?.votedUsers || 0) / totalVotes) * 100,
                        )
                      : 0;
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        overflow: "hidden",
                        background: isThisVoted ? "#F0F7FF" : "#FFF",
                      }}
                    >
                      {votedOptionId && (
                        <div
          ...
          {mediaUrls.length > 0 && (
            <div
              ...
              style={{
                width: isMobile ? "100%" : 277,
                height: isMobile ? 250 : 275,
                borderRadius: 12,
                overflow: "hidden",
              ...
              <img
                ...
                onClick={() =>
                  navigate("/critique-create", {
                    state: { replyToId: post.id, replyToContent: post.text },
                  })
                }
                style={{ width: 20, cursor: "pointer" }}
              />
        ...
        {!isPoll && (
          <>
            <div
              style={{
                borderTop: "0.5px solid rgba(148, 148, 148, 0.25)",
                margin: "20px 0",
              }}
            />
                ...
                {post.sources.map((s, i) => (
                  ...
                  >
                    ...
                    <div style={{ overflow: "hidden" }}>
                      ...
                      <a
                        href={formatExternalLink(s.link)}
                        target="_blank"
