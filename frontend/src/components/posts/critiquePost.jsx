  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ color: "#1C274C", marginBottom: 20 }}>Do you want to delete this critique?</h3>
...
export default function CritiquePost({ postId, initialData }) {
  ...
  const handleEditSave = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/posts/${post.id || post._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/posts/${post.id || post._id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const sources = Array.isArray(post.sources) && post.sources.length > 0 
    ? post.sources 
    : (post.sourceLink ? [{ name: post.sourceName || "Source", link: post.sourceLink }] : []);
      ...
      <PostCard ref={postRef}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {originalPost?.author && (
              <>
                <span style={{ fontSize: 16, color: "#1C274C", margin: "0 4px" }}>on</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <img src={originalPost.author.image ? (originalPost.author.image.startsWith('http') ? originalPost.author.image : `${BACKEND_URL}${originalPost.author.image}`) : profileDefault} alt="" style={{ width: 41, height: 41, borderRadius: 999, objectFit: "cover" }} />
                  <div>
                    <span style={{ fontSize: 16, fontWeight: 400, color: "#1C274C" }}>{originalPost.author.name}</span>
                    <span style={{ fontSize: 16, fontWeight: 500, color: "#1C274C" }}>'s post</span>
                  </div>
                </div>
              </>
            )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: "#FFC847", textTransform: "lowercase" }}>{post.type}</div>
          </div>
        </div>
        ...
        {originalPost && (
          <div
            ...
            style={{
              marginTop: 10, padding: "0 12px", display: "flex", alignItems: "center", justifyContent: "space-between",
              background: "#F6F6F6", border: "0.5px solid #1C274C", height: 49, borderRadius: 13, fontSize: 16, color: "#1C274C", cursor: "pointer",
            }}
          >
            <div style={{ overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis", maxWidth: "calc(100% - 85px)" }}>
              {originalPost.text}
            </div>
            <button style={{ background: "transparent", border: "none", color: "#1C274C", fontSize: 16, fontWeight: 500, cursor: "pointer" }}>
              See Post
            </button>
          </div>
        )}
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 20 }}>
          ...
          {mediaList.length > 0 && !isEditing && (
            <div 
              style={{ width: 277, height: 275, borderRadius: 12, overflow: "hidden", flexShrink: 0, position: "relative", cursor: "zoom-in" }}
              onClick={() => setMaximizedIndex(imageIndex)}
            >
              {mediaList[imageIndex].match(/\.(mp4|webm|ogg)$/i) ? (
                <video src={mediaList[imageIndex]} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              ) : (
                <img src={mediaList[imageIndex]} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              )}
