...
export default function PollPost({ postId, initialData }) {
  ...
  const isAuthor =
    user && String(user.id || user._id) === String(post?.authorId);
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/posts/${cid}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const recordView = async (sessionKey) => {
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/posts/${currentPostId}/view`,
        {
          method: "POST",
      ...
      if (res.ok) {
        ...
        setPost((prev) =>
          prev ? { ...prev, viewsCount: data.viewsCount } : null,
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const handleVote = async (e, optionId) => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/posts/${currentPostId}/vote`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      showToastFn("Error voting", "error");
    }
  };
  ...
  const toggleSave = async (e) => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/posts/${currentPostId}/save`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const totalVotes = pollData.reduce(
    (sum, o) => sum + (o._count?.votedUsers || 0),
    0,
  );
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            />
            <div>
              <div style={{ fontSize: 16, fontWeight: 400, color: "#1C274C" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 500, color: "#1C274C" }}>
              {post.viewsCount || 0} views
            </div>
            <div style={{ fontSize: 14, fontWeight: 500, color: "#1C274C" }}>
              {totalVotes} voted
            </div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "#FFC847" }}>
              poll
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 15, alignItems: "center" }}>
            ...
            <div style={{ position: "relative" }}>
              <img
                src={isAuthor ? MenuIcon : FlagIcon}
                onClick={(e) => {
                  e.stopPropagation();
                  isAuthor
              ...
              {showFlagPopup && !isAuthor && (
                ...
                >
                  {["Invalid resources", "Inappropriate language", "Spam"].map(
                    (opt) => (
                      <div
                        key={opt}
                        onClick={(e) => {
                          e.stopPropagation();
                          handleSubmitPostReport(opt);
                        }}
                        style={{
                          padding: "8px",
                          cursor: "pointer",
