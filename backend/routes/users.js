...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (localOnly) {
      const existingUser = localUsers.findByEmailOrUsername(email) || localUsers.findByEmailOrUsername(username);
      if (existingUser) {
        return res.status(400).json({ error: "Email or username already exists" });
      }

      const hashedPassword = await bcrypt.hash(password, 10);
      const user = localUsers.createUser({
        name,
        email,
        username,
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (localOnly) {
      const user = localUsers.findByEmailOrUsername(emailOrUsername);
...
router.get("/search", async (req, res) => {
  ...
  try {
    if (localOnly) {
      return res.json(localUsers.searchUsers(q, requesterId));
    }
...
router.get("/list", async (req, res) => {
  try {
    ...
    if (localOnly) {
      return res.json(localUsers.listUsers(limit));
    }
...
router.put("/:userId", memoryUpload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (localOnly) {
      const updatedUser = localUsers.updateUser(userId, updateData);
...
router.put("/:userId/password", async (req, res) => {
  ...
  try {
    if (localOnly) {
      const user = localUsers.findById(userId);
      if (!user) return res.status(404).json({ error: "User not found" });
      ...
      localUsers.updatePassword(userId, hashedNewPassword);
...
router.get("/:userId/posts", async (req, res) => {
  try {
    const requesterId = getRequesterId(req);
    if (localOnly) {
      return res.json(await dataService.getUserPosts(req.params.userId, requesterId));
    }
