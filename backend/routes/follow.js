...
router.post("/:userId/follow", async (req, res) => {
  ...
  try {
    if (localOnly) {
      const result = localUsers.followUser(followerId, userId);
      if (result.reason === "not-found") return res.status(404).json({ error: "User not found" });
      if (result.reason === "exists") return res.status(400).json({ error: "Already following" });
...
router.post("/:userId/unfollow", async (req, res) => {
  ...
  try {
    if (localOnly) {
      localUsers.unfollowUser(followerId, userId);
