...
router.post("/", memoryUpload.array("media"), async (req, res) => {
  try {
    ...
    const sourceValidation = validateSources({ type: "article", text: `${title} ${content}`, sources: parsedSources });
    if (!sourceValidation.valid) {
      return res.status(422).json({ error: "Article sources did not pass verification", reasons: sourceValidation.reasons });
    }
